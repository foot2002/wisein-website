...
// Supabase는 일반적으로 aws-0 리전을 사용합니다
const possibleHosts = [
  `aws-0-${projectId}.pooler.supabase.com`,  // Connection pooling
  `aws-0.${projectId}.supabase.co`,          // Direct connection
  `db.${projectId}.supabase.co`,             // Alternative format
  `${projectId}.supabase.co`,                 // Fallback
];
...
// 비밀번호 확인
const password = supabaseDbPassword || supabaseServiceKey;
if (!password) {
  console.error('❌ SUPABASE_DB_PASSWORD or SUPABASE_SERVICE_ROLE_KEY required');
  console.error('💡 .env.local에 SUPABASE_DB_PASSWORD=wiseinc1004! 추가');
  process.exit(1);
}
...
async function createTables() {
  ...
  console.log('📋 관리자 페이지 DB 구조와 동일하게 생성합니다.\n');
  ...
  for (const dbHost of possibleHosts) {
    ...
    try {
      ...
      for (let i = 0; i < statements.length; i++) {
        ...
        try {
          ...
          if (error.message.includes('already exists') || 
            ...
            console.error(`⚠️  오류: ${error.message.substring(0, 80)}`);
          }
        }
      }

      console.log(`\n✅ 완료! 성공: ${successCount}, 건너뜀: ${skipCount}\n`);
      ...
      const expectedTables = ['portfolio', 'blog', 'inquiries', 'announcements', 'press_releases', 'newsletter_subscribers'];
      ...
      if (missingTables.length === 0) {
        ...
        console.log(`\n⚠️  다음 테이블이 누락되었습니다: ${missingTables.join(', ')}`);
      }
      ...
      if (error.message.includes('ENOTFOUND') || error.message.includes('ECONNREFUSED')) {
        ...
        console.log('⏱️  연결 시간 초과, 다음 호스트 시도...\n');
        continue;
      } else {
  ...
  console.error('   또는 SQL Editor를 사용하세요: https://app.supabase.com/project/kehgopppnjqxjfjuiyvy/sql/new');
  process.exit(1);
}
